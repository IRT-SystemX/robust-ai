...
ruff check robustAI/ > scripts/ruff_report.txt 

# Launch flake8
flake8 robustAI  --config=scripts/.flake8 --format=html --htmldir=_static/flake8 --exit-zero 
...
# Launch pylint
pylint robustAI --rcfile=scripts/.pylintrc --output-format=text --exit-zero | tee _static/pylint/pylint.txt
PYLINT_SCORE=$(sed -n 's/^Your code has been rated at \([-0-9.]*\)\/.*/\1/p' _static/pylint/pylint.txt)
pylint robustAI --rcfile=scripts/.pylintrc --output-format=pylint_gitlab.GitlabPagesHtmlReporter --exit-zero > _static/pylint/index.html 
